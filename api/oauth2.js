...
let guildsinfo = JSON.parse(await guildsjson.text());
  ...
  if (newsettings.blacklistedusers.includes(userinfo.id)) {
    return res.redirect(failedcallback + "?err=BLACKLISTEDUSER");
  }
        ...
        if (!await db.get("users-" + userinfo.id)) {
          if (newsettings.api.client.allow.newusers == true) {
            ...
            if (await accountjson.status == 201) {
              ...
              await db.set("global-" + userinfo.id, userinfo.global_name)
              await db.set("avatar-" + userinfo.id, userinfo.avatar)
